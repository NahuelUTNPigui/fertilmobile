<script>
        ...
        for (let i = 0; i < inseminacionesprocesar.length; i++) {
            ...
            if (ins.madre != "" && ins.fecha != "" && ins.pajuela != "") {
                try {
                    ...
                    //    skipTotal:true
                    //})
                    let recordmadre = animales.filter(
                        (m) => m.caravana == ins.madre,
                    )[0];
                    let recordspadre = animales.filter(
                        (m) => m.caravana == ins.pajuela,
                    )[0];
                    ...
                    let datains = {
                        ...
                        pajuela: ins.pajuela,
                        padre:
                            recordspadre.items.length != 0
                                ? recordspadre.items[0].id
                                : "",
                        fechaparto:
                            addDays(ins.fecha, 280)
                                .toISOString()
                                .split("T")[0] + " 03:00:00",
                        fechainseminacion:
                            ins.fecha.toISOString().split("T")[0] + " 03:00:00",
                        active: true,
                        cab: caboff.id,
                    ...
                    let nuevopadre =
                        recordspadre.items.length != 0
                            ? recordspadre.items[0].id.split("_").length > 1
                            : false;
                    let comando = {
                        tipo: "add",
    ...
    async function procesarOnline(inseminacionesprocesar) {
        ...
        for (let i = 0; i < inseminacionesprocesar.length; i++) {
            ...
            if (ins.madre != "" && ins.fecha != "" && ins.pajuela != "") {
                try {
                    let lista_madre = animales.filter(
                        (m) => m.caravana == ins.madre && m.cab == caboff.id,
                    );
                    ...
                    let lista_padre = animales.filter(
                        (p) => p.caravana == ins.pajuela && p.cab == caboff.id,
                    );
                    ...
                    let datains = {
                        ...
                        fechaparto:
                            addDays(new Date(ins.fecha), 280)
                                .toISOString()
                                .split("T")[0] + " 03:00:00",
                        fechainseminacion:
                            new Date(ins.fecha).toISOString().split("T")[0] +
                            " 03:00:00",
                        active: true,
                        cab: caboff.id,
                        observacion: ins.observacion,
                    ...
                    if (i_idx == -1) {
                        ...
                        await pb
                            .collection("inseminacion")
                            .update(inseminaciones[i_idx].id, datamod);
