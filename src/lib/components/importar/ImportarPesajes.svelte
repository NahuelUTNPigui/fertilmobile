<script>
    ...
    async function procesarArchivoOnline(pesajesprocesar) {
        ...
        for (let i = 0; i < pesajesprocesar.length; i++) {
            ...
            let ans = animales.filter(
                (a) => a.caravana == pe.caravana && a.cab == caboff.id,
            );
            if (ans.length == 0) {
                continue;
            }
            ...
            try {
                ...
                if (p_idx == -1) {
                    ...
                    await pb
                        .collection("pesaje")
                        .update(pesajes[p_idx].id, datamod);
                }
            } catch (err) {
                console.error(err);
                errores = true;
    ...
    async function procesarArchivoOffline(pesajeprocesar) {
        ...
        for (let i = 0; i < pesajesprocesar.length; i++) {
            ...
            try {
                let recordanimal = await pb
                    .collection("animales")
                    .getFirstListItem(
                        `caravana="${pe.caravana}" && cab='${cab.id}' && active = True`,
                    );
                ...
                await pb
                    .collection("animales")
                    .update(recordanimal.id, datamod);
            } catch (err) {
                console.error(err);
                errores = true;
            }
