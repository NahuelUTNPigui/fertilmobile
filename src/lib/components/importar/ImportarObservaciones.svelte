        ...
        for (let i = 0; i < observacionesprocesa.length; i++) {
            ...
            try {
                const record = await pb
                    ...
                    .getFirstListItem(
                        `fecha="${ob.fecha + " 03:00:00"}" && animal="${an.id}"`,
                        {},
                    );
    ...
    async function procesarArchivoOnline(observacionesprocesa) {
        ...
        for (let i = 0; i < observacionesprocesa.length; i++) {
            ...
            let ans = animales.filter(
                (a) => a.caravana == ob.caravana && a.cab == caboff.id,
            );
            if (ans.length == 0) {
                continue;
            }
            ...
            let fecha =
                new Date(ob.fecha).toISOString().split("T")[0] + " 03:00:00";
            let dataadd = {
                animal: ans[0].id,
                categoria: ob.categoria,
                fecha,
            ...
            if (o_idx == -1) {
                ...
                try {
                    await pb
                        .collection("observaciones")
                        .update(observaciones[o_idx].id, datamod);
                    observaciones[o_idx] = {
                        ...observaciones[o_idx],
                        ...datamod,
                    };
    ...
    async function procesarArchivo() {
        ...
        for (const [key, value] of Object.entries(sheetobservaciones)) {
            ...
            if (observacioneshashmap[tail]) {
                ...
                if (firstLetter == "B") {
                    observacioneshashmap[tail].categoria =
