<script>
    ...
    import { addNewTactoSQL, setTactosSQL } from "$lib/stores/sqlite/dbeventos";
    ...
    async function editarTactoOffline() {
        try {
            ...
            tactos.sort((t1, t2) =>
                new Date(t1.fecha) > new Date(t2.fecha) ? -1 : 1,
            );
            onChangeTactos();
            await setTactosSQL(db, tactos);
    ...
    function onChangeTactos() {
        tactosrows = tactos
            .filter((t) => t.animal == id)
            .sort((t1, t2) => (new Date(t1.fecha) < new Date(t2) ? 1 : -1));
    }
    onMount(async () => {
        id = $page.params.slug;
        prenada = prenadaori;
    ...
    >
        ...
        <div class="modal-action justify-start">
            ...
            <button
                ...
                onclick={() => nuevoTactoAnimal.close()}>Cerrar</button
            >
        </div>
    </div>
</dialog>
