<script>
    ...
    function onSelectAnimalNacimiento(esmadre) {
        let a = animales.filter((an) =>
            esmadre
                ? an.id == nacimiento.madrenac
                : an.id == nacimiento.padrenac,
        )[0];
        if (esmadre) {
...
{#if modedebug}
    ...
    {#if isEmpty(nacimiento.fechanac)}
        <span class="label-text"> fecha vacua </span>
    {/if}
    {#if isEmpty(nacimiento.madrenac)}
        <span class="label-text"> madre vacia </span>
    {/if}
    {#if !nacimiento.botonhabilitadonac}
        <span class="label-text"> mal boton </span>
    {/if}
{/if}
...
<div class="modal-action justify-start">
    <form method="dialog">
        ...
        <button class="btn btn-success text-white" onclick={guardarNacimiento}
            >Guardar</button
        >
    </form>
</div>
