    ...
    */
    ...
    async function guardarNacimientoOnline(dataparicion) {
        ...
        try {
            ...
            Swal.fire(
                "Éxito guardar",
                "Se pudo guardar la parición con exito",
                "success",
            );
            ...
            Swal.fire(
                "Error guardar",
                "No se pudo guardar la parición",
                "error",
            );
            return;
        }
    ...
    async function guardarTratAnimal(idprov) {
        ...
        //Sino tengo que crear un comando
            ...
            Swal.fire(
                "Éxito guardar",
                "Se pudo guardar el tratamiento",
                "success",
            );
        }
    }
    async function guardarTrat() {
        ...
        //Animal existente
            ...
            let tts = tipotratamientos.filter(
                (t) => t.id == tratamiento.tipotrat,
            );
            let data = {
            ...
            if (coninternet.connected) {
                ...
                try {
                    ...
                    Swal.fire(
                        "Error guardar",
                        "No se pudo guardar el tratamiento con exito",
                        "error",
                    );
                }
            } else {
    ...
    async function guardarInseminacion() {
        ...
        if (agregaranimal) {
            ...
            if (coninternet.connected) {
                ...
                try {
                    let animalins = animales.filter(
                        (an) => an.id == inseminacion.idanimalins,
                    )[0];
                    let record = await pb
                ...
                let nuevoanimal =
                    inseminacion.idanimalins.split("_")[0] == "nuevo";
                data.id = idprov;
                let comando = {
                    tipo: "add",
                    coleccion: "inseminacion",
    ...
    async function guardarServicioAnimal(idprov) {
        ...
        if (coninternet.connected) {
            ...
            Swal.fire(
                "Éxito guardar",
                "Se pudo guardar el servicio con exito",
                "success",
            );
            totaleventos.servicios += 1;
        }
        ...
        if (agregaranimal) {
            ...
            if (coninternet.connected) {
                ...
                try {
                    ...
                    Swal.fire(
                        "Error guardar",
                        "No se pudo guardar el servicio con éxito",
                        "error",
                    );
                }
            } else {
    ...
    async function guardarObservacionAnimal(idprov) {
        ...
        if (coninternet.connected) {
            ...
            Swal.fire(
                "Éxito guardar",
                "Se pudo guardar la observación con exito",
                "success",
            );
            totaleventos.observaciones += 1;
        }
    ...
    async function guardarObservacion() {
        ...
        //Animal seleccionado
            ...
            if (coninternet.connected) {
                ...
                try {
                    let a = animales.filter(
                        (an) => an.id == observacion.animalobs,
                    )[0];
                    let record = await pb
                ...
                let nuevoanimal =
                    observacion.animalobs.split("_")[0] == "nuevo";
                data.id = idprov;
                let comando = {
                    tipo: "add",
                    coleccion: "observaciones",
    ...
    async function oldUpdateLocalSQL(db) {
        ...
        onChangeAnimales();
        totaleventos.tactos = datauser.tactos.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.inseminaciones = datauser.inseminaciones.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.nacimientos = datauser.nacimientos.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.tratamientos = datauser.trats.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.observaciones = datauser.observaciones.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.pesajes = datauser.pesajes.filter(
            (t) => t.expand.animal.cab == caboff.id,
        ).length;
        totaleventos.servicios = datauser.servicios.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.lotes = datauser.lotes.filter(
            (t) => t.cab == caboff.id,
        ).length;
        totaleventos.rodeos = datauser.rodeos.filter(
            (t) => t.cab == caboff.id,
        ).length;
        ...
        tipotratamientos = datauser.tipostrat.filter(
    ...
    async function getLocalSQL(db) {
        ...
        totaleventos.servicios = data.servicios.lista.filter(
            (t) => t.cab == caboff.id,
        ).length;
    ...
    {:else}
        ...
        <CardBase titulo="Bienvenido a Creciente Fértil" cardsize="max-w-5xl">
            <div class="grid grid-cols-1 gap-6">
                <a class={classbutton} href="/establecimiento">
                    <svg
